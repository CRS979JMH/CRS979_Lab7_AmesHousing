...
if df is not None:
    ...
    def user_input_features():
        Overall Qual = st.sidebar.slider('Overall Quality', 1, 10, 5)
        Gr Liv Area = st.sidebar.number_input('Above Ground Living Area (sq ft)', min_value=500, max_value=5000, value=1500)
        Garage Cars = st.sidebar.slider('Garage Cars', 0, 4, 2)
        Total Bsmt SF = st.sidebar.number_input('Total Basement Area (sq ft)', min_value=0, max_value=3000, value=1000)
        Full Bath = st.sidebar.slider('Number of Full Bathrooms', 1, 5, 2)
        Year Built = st.sidebar.number_input('Year Built', min_value=1800, max_value=2025, value=2000)
